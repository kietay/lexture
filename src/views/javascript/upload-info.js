...
$(document).ready(function () {
    ...
    intervalId = setInterval(function(){fakeProgress(currTime)}, 500);

    // Gather all form inputs into an array when SAVE & SUBMIT is clicked
    $('#video-info-form').submit(function(event) {
        var formValues = $(this).serializeArray();

        // Note for Kieran, when using TAGS:
        // The TAGS come in as a string, are split by comma, and then spaces/newlines on either side are trimmed
        // However, TAGS are not required (unless you want them to be), so if the TAGS form is left blank,
        // we will get back just a single empty string in an array.
